...
public class IndexFiles {
	...
	boolean stem;
	/** Index all text files under a directory. */
	public IndexFiles(String indexDestiny, String documentsPath, boolean stop, boolean stem){
		this.stop = stop;
		this.stem = stem;
		
		boolean create = true;
		//    for(int i=0;i<_args.length;i++) {
		//      if ("-index".equals(_args[i])) {
		//        indexPath = _args[i+1];
		//        i++;
		//      } else if ("-docs".equals(_args[i])) {
		//        docsPath = _args[i+1];
		//        i++;
		//      } else if ("-update".equals(_args[i])) {
		//        create = false;
		//      }
		//    }
		...
		try {
			...
			Analyzer analyzer = new StemAnalyzer(stop,stem);
